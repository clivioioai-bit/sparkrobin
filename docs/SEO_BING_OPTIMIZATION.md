...
- ✅ 添加了 `gemini-omni-flash-video-generator` 和 `ads-landing` 页面
- ✅ 优化了优先级（priority）设置：
  - 首页: 1.0
  - 核心功能页: 0.95
  - 次要功能页: 0.9
...
3. Bing 会自动访问 `https://omniflashai.io/BingSiteAuth.xml` 进行验证
...
2. 进入 "Sitemaps" 部分
3. 提交以下 sitemap URL：
   - `https://omniflashai.io/sitemap.xml` (主索引)
   - `https://omniflashai.io/sitemap-en.xml`
   - `https://omniflashai.io/sitemap-ar.xml`
   - `https://omniflashai.io/sitemap-ja.xml`
   - `https://omniflashai.io/sitemap-ru.xml`

### 3. 验证配置

#### 检查 robots.txt
访问: `https://omniflashai.io/robots.txt`
...
#### 检查 sitemap
访问: `https://omniflashai.io/sitemap.xml`

应该看到指向所有语言特定 sitemap 的索引。

#### 检查 Bing 验证
访问: `https://omniflashai.io/BingSiteAuth.xml`
...
curl -X POST "https://ssl.bing.com/webmaster/api.svc/json/SubmitUrl?apikey=YOUR_API_KEY" \
  ...
  -d '{"siteUrl":"https://omniflashai.io","url":"https://omniflashai.io/gemini-omni-flash-video-generator"}'
```
...
2. 提交 sitemap URL: `https://omniflashai.io/sitemap.xml`
3. 或手动提交重要页面的 URL
...
3. **检查特定页面**:
   ```
   site:omniflashai.io/gemini-omni-flash-video-generator
   ```

### 测试 Bing 验证

1. **Meta 标签验证**:
   - 访问 `https://omniflashai.io`
   - 查看页面源代码
...
1. **主 sitemap**:
   ```
   https://omniflashai.io/sitemap.xml
   ```
