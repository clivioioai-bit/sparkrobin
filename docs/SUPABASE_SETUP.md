...
## 10. 配置 Storage Buckets

应用需要 Supabase Storage 来存储上传的图片和视频文件。

### 10.1 创建 Storage Bucket

1. 在 Supabase Dashboard 中，进入 **Storage**
2. 点击 **New bucket**
3. 配置 bucket：
   - **Name**: `videos`（必须使用这个名称）
   - **Public bucket**: ✅ 勾选（允许公开访问）
   - 点击 **Create bucket**

### 10.2 配置 Storage Policies（可选）

如果需要更细粒度的权限控制：

1. 进入 **Storage** → **Policies**
2. 为 `videos` bucket 创建策略：
   - **Policy name**: `Allow authenticated uploads`
   - **Allowed operation**: `INSERT`
   - **Policy definition**: 
     ```sql
     (bucket_id = 'videos'::text) AND (auth.role() = 'authenticated')
     ```
...
测试上传功能：
1. 在应用中尝试上传图片或视频
2. 如果看到 "Bucket not found" 错误，检查：
   - Bucket 名称是否为 `videos`（区分大小写）
   - Bucket 是否已创建
   - 是否勾选了 "Public bucket"
