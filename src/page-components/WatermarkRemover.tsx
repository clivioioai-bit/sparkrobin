...
const WatermarkRemover: React.FC = () => {
    ...
    try {
      ...
      const poll = async () => {
        ...
        if (data.status === 'failed' || data.status === 'error') {
          throw new Error(data.error_message || data.error || 'Task failed');
        }
  ...
  const seoDesc = 'Sora3 watermark remover: remove Sora watermarks from Sora-hosted videos in seconds. Commercial use, API-ready, works with public sora.chatgpt.com links.';

  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title="Sora3 Watermark Remover | Remove Sora Watermarks"
        description={seoDesc}
                  ...
                  <div className="space-y-4">
                    <div className="bg-accent/10 border-2 border-accent/20 rounded-xl p-4">
                      ...
                      <p className="text-sm text-accent-foreground/80">
                        This is a sample video generated by Sora3 AI. Upload your own Sora video to remove watermarks!
                      </p>
                    </div>
