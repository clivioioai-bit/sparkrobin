...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signUp = async (email: string, password: string, metadata?: { full_name?: string }) => {
    ...
    try {
      ...
      const baseUrl = isProduction ? (process.env.NEXT_PUBLIC_APP_URL || 'https://sora3ai.io') : window.location.origin
  ...
  const signInWithGoogle = async () => {
    ...
    if (isProduction) {
      baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://sora3ai.io';
    } else {
