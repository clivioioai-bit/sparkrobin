...
export async function createCheckoutSession(
  ...
  if (!response.ok) {
    ...
    if (message.includes('API key') || message.includes('not configured')) {
      error.message = 'Payment service is temporarily unavailable. Please contact support@sparkrobinai.io for assistance.';
    } else if (message.includes('安全错误') || message.includes('Security')) {
      error.message = 'Payment service configuration error. Please contact support@sparkrobinai.io.';
