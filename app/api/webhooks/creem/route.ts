...
async function handleSubscriptionCreated(data: any) {
  ...
  const handlerId = `sub_created_${Date.now()}_${Math.random().toString(36).substring(7)}`;
  
  console.log(`[WEBHOOK-${handlerId}] ========================================`);
  console.log(`[WEBHOOK-${handlerId}] Processing subscription.created event`);
  console.log(`[WEBHOOK-${handlerId}] Timestamp: ${new Date().toISOString()}`);
  console.log(`[WEBHOOK-${handlerId}] Input data:`, {
    subscription_id,
    customer_id,
    plan_id,
    status,
  ...
  // 验证必需字段
  if (!customer_id) {
    console.error(`[WEBHOOK-${handlerId}] ❌ CRITICAL: customer_id is missing!`);
    console.error(`[WEBHOOK-${handlerId}] Cannot proceed without customer_id`);
    return { success: false, error: 'MISSING_CUSTOMER_ID', handlerId };
  }

  if (!subscription_id) {
    console.warn(`[WEBHOOK-${handlerId}] ⚠️ WARNING: subscription_id is missing, subscription record may not be linkable`);
  }
  ...
  console.log(`[WEBHOOK-${handlerId}] Plan config:`, planConfig ? { 
    id: planConfig.id, 
    name: planConfig.name, 
    credits: planConfig.credits,
    groupId: planConfig.groupId,
    category: planConfig.category
  } : 'null');
  console.log(`[WEBHOOK-${handlerId}] Credit amount:`, { 
  ...
  console.log(`[WEBHOOK-${handlerId}] Step 1: Upserting subscription record...`);
  const subscriptionIdColumn = await resolveSubscriptionIdColumn(supabaseAdmin);
  console.log(`[WEBHOOK-${handlerId}] Using subscription ID column: ${subscriptionIdColumn}`);
    ...
    try {
      ...
      if (typeof dateValue === 'string') {
        ...
        if (isNaN(parsed.getTime())) {
          console.warn(`[WEBHOOK-${handlerId}] Invalid date string: ${dateValue}`);
          return null;
        }
      ...
      if (dateValue instanceof Date) {
        if (isNaN(dateValue.getTime())) {
          console.warn(`[WEBHOOK-${handlerId}] Invalid Date object`);
          return null;
        }
      ...
      if (typeof dateValue === 'number') {
        const parsed = new Date(dateValue);
        if (isNaN(parsed.getTime())) {
          console.warn(`[WEBHOOK-${handlerId}] Invalid timestamp: ${dateValue}`);
          return null;
        }
        return parsed.toISOString();
      }
      
      console.warn(`[WEBHOOK-${handlerId}] Unknown date type: ${typeof dateValue}`, dateValue);
      return null;
    } catch (error) {
      console.error(`[WEBHOOK-${handlerId}] Error normalizing date:`, error, dateValue);
      return null;
    }
  };
  
  const normalizedPeriodStart = normalizeDate(data.current_period_start);
  const normalizedPeriodEnd = normalizeDate(data.current_period_end);
  
  console.log(`[WEBHOOK-${handlerId}] Normalized dates:`, {
    original_start: data.current_period_start,
    normalized_start: normalizedPeriodStart,
    original_end: data.current_period_end,
    normalized_end: normalizedPeriodEnd
  });
  
  // 先检查是否已存在订阅记录
  const { data: existingSubscription, error: checkError } = await supabaseAdmin
    .from('user_subscriptions')
    .select('id, plan_type, plan_status, user_id')
    .eq('user_id', customer_id)
    .maybeSingle();

  if (checkError) {
    console.error(`[WEBHOOK-${handlerId}] ❌ Failed to check existing subscription:`, {
      error: checkError,
  ...
  const isUpdate = !!existingSubscription;

  if (existingSubscription) {
    // 更新现有订阅
    console.log(`[WEBHOOK-${handlerId}] Updating existing subscription record (ID: ${existingSubscription.id})...`);
    const updateData: any = {
      plan_status: status || 'active',
      plan_type: planConfig?.groupId || planConfig?.id || 'basic',
    ...
    console.log(`[WEBHOOK-${handlerId}] Update data:`, JSON.stringify(updateData, null, 2));
    ...
    // 创建新订阅
    console.log(`[WEBHOOK-${handlerId}] Creating new subscription record...`);
    ...
    console.log(`[WEBHOOK-${handlerId}] Insert data:`, JSON.stringify(insertData, null, 2));
  ...
  if (subscriptionError) {
    console.error(`[WEBHOOK-${handlerId}] ❌❌❌ CRITICAL: Failed to ${isUpdate ? 'update' : 'insert'} subscription!`);
    console.error(`[WEBHOOK-${handlerId}] Error details:`, {
    ...
    console.error(`[WEBHOOK-${handlerId}] Operation: ${isUpdate ? 'UPDATE' : 'INSERT'}`);
    console.error(`[WEBHOOK-${handlerId}] Customer ID: ${customer_id}`);
    console.error(`[WEBHOOK-${handlerId}] Subscription ID: ${subscription_id || 'N/A'}`);
    
    // 检查是否是列不存在的错误
    if (subscriptionError.code === '42703' || subscriptionError.code === 'PGRST102' || 
        (subscriptionError.message && subscriptionError.message.toLowerCase().includes('column'))) {
      console.error(`[WEBHOOK-${handlerId}] ⚠️ Possible missing column error!`);
      console.error(`[WEBHOOK-${handlerId}] Please run database migration: database/add-subscription-columns.sql`);
    }
    
    // 检查是否是 RLS 策略错误
    if (subscriptionError.code === '42501' || 
        (subscriptionError.message && subscriptionError.message.toLowerCase().includes('permission'))) {
      console.error(`[WEBHOOK-${handlerId}] ⚠️ Possible RLS policy error!`);
      console.error(`[WEBHOOK-${handlerId}] Check RLS policies for user_subscriptions table`);
    }
    
    // 不抛出异常，记录错误但继续处理
    console.log(`[WEBHOOK-${handlerId}] Continuing despite subscription ${isUpdate ? 'update' : 'insert'} error`);
  } else {
    console.log(`[WEBHOOK-${handlerId}] ✅ Subscription ${isUpdate ? 'updated' : 'created'} successfully`);
    console.log(`[WEBHOOK-${handlerId}] Subscription data:`, JSON.stringify(subscriptionData, null, 2));
    
    // 验证数据确实被保存
    if (subscriptionData && typeof subscriptionData === 'object' && 'id' in subscriptionData) {
      console.log(`[WEBHOOK-${handlerId}] Verifying subscription record was saved...`);
      const { data: verifyData, error: verifyError } = await supabaseAdmin
        .from('user_subscriptions')
        .select('id, user_id, plan_type, plan_status, ' + subscriptionIdColumn)
        .eq('id', (subscriptionData as any).id)
        .single();
      
      if (verifyError) {
        console.error(`[WEBHOOK-${handlerId}] ⚠️ Verification failed - cannot read back subscription:`, verifyError);
      } else if (verifyData && typeof verifyData === 'object') {
        const verifyRecord = verifyData as any;
        console.log(`[WEBHOOK-${handlerId}] ✅ Verification successful - subscription record confirmed in database:`, {
          id: verifyRecord.id,
          user_id: verifyRecord.user_id,
          plan_type: verifyRecord.plan_type,
          plan_status: verifyRecord.plan_status,
          subscription_id: verifyRecord[subscriptionIdColumn]
        });
      } else {
        console.error(`[WEBHOOK-${handlerId}] ⚠️ Verification failed - subscription record not found after ${isUpdate ? 'update' : 'insert'}`);
      }
    }
  }

  // 2. 更新用户表的订阅信息
  console.log(`[WEBHOOK-${handlerId}] Step 2: Updating user subscription info...`);
  ...
  console.log(`[WEBHOOK-${handlerId}] User update data:`, JSON.stringify(userUpdateData, null, 2));
  
  const { data: userUpdateResult, error: userUpdateError } = await supabaseAdmin
    .from('users')
  ...
  if (userUpdateError) {
    console.error(`[WEBHOOK-${handlerId}] ❌ Failed to update user subscription:`, {
    ...
    // 不抛出异常，记录错误但继续处理
    console.log(`[WEBHOOK-${handlerId}] Continuing despite user update error`);
  } else {
    console.log(`[WEBHOOK-${handlerId}] ✅ User subscription info updated successfully:`, {
      id: userUpdateResult?.id,
      email: userUpdateResult?.email,
      subscription_plan: userUpdateResult?.subscription_plan,
      subscription_status: userUpdateResult?.subscription_status,
      subscription_end_date: userUpdateResult?.subscription_end_date
    });
  }

  // 3. 发放订阅积分（如果未跳过）
  console.log(`[WEBHOOK-${handlerId}] Step 3: Credit activation check:`, {
    skipCredits,
    creditAmount,
  ...
  if (!skipCredits && creditAmount > 0) {
    try {
      ...
      if (alreadyCredited) {
        ...
        try {
          const creditResult = await creditCredits(
            customer_id,
            creditAmount,
          ...
          console.log(`[WEBHOOK-${handlerId}] ✅ Subscription credits credited`, {
            userId: customer_id,
            amount: creditAmount,
            subscriptionId: subscription_id,
            snapshot: {
              credits_balance: creditResult.balance,
              credits_total: creditResult.total,
              credits_spent: creditResult.spent
            }
          });
        } catch (creditErr) {
          console.error(`[WEBHOOK-${handlerId}] ❌ Failed to credit subscription:`, {
            error: creditErr instanceof Error ? creditErr.message : 'Unknown error',
      ...
      console.error(`[WEBHOOK-${handlerId}] Exception updating credits:`, {
  ...
  // 4. 最终验证和总结
  console.log(`[WEBHOOK-${handlerId}] Step 4: Final verification...`);
  const finalSummary = {
    subscriptionSaved: !!subscriptionData && !subscriptionError,
    userUpdated: !!userUpdateResult && !userUpdateError,
    creditsAdded: !skipCredits && creditAmount > 0,
    subscriptionId: subscriptionData?.id || null,
    planType: subscriptionData?.plan_type || null,
    planStatus: subscriptionData?.plan_status || null
  };

  console.log(`[WEBHOOK-${handlerId}] ========================================`);
  console.log(`[WEBHOOK-${handlerId}] Final Summary:`, finalSummary);
  console.log(`[WEBHOOK-${handlerId}] ========================================`);
  ...
  if (subscriptionId && planConfig?.credits) {
    ...
    if (!alreadyCredited) {
      try {
        ...
        console.log('[WEBHOOK] Flex credits credited OK', { 
          userId: finalUserId, 
          amount: creditAmount, 
          snapshot: {
            credits_balance: creditResult.balance,
...
async function handleCheckoutCompleted(checkout: any) {
  ...
  if (product.billing_type === 'recurring' && subscription) {
    ...
    console.log(`[WEBHOOK-${handlerId}] Subscription checkout - plan identification:`, {
      planIdFromMetadata,
      productId: product.id,
      finalPlanId: plan_id,
    ...
    // 提取日期字段（支持多种可能的字段名）
    const extractDate = (obj: any, ...fieldNames: string[]): string | null => {
    ...
    const currentPeriodStart = extractDate(
      subscription,
    ...
    const currentPeriodEnd = extractDate(
      subscription,
    ...
    console.log(`[WEBHOOK-${handlerId}] Extracted subscription dates:`, {
      currentPeriodStart,
      currentPeriodEnd,
    ...
    const subscriptionResult = await handleSubscriptionCreated({
      subscription_id: subscription.id,
      customer_id: userId, // 使用 Supabase 用户ID
      plan_id: plan_id, // 使用 metadata 中的 planId 或 product.id
      status: subscription.status || 'active', // 确保有默认状态
    ...
    if (subscriptionResult && !subscriptionResult.success) {
      console.error(`[WEBHOOK-${handlerId}] ❌ Subscription creation failed:`, subscriptionResult);
    }
  ...
  if (!alreadyCredited && creditAmount > 0) {
    ...
    try {
      ...
      console.log(`[WEBHOOK-${handlerId}] 🔍 Crediting flex credits...`);
      ...
      try {
        ...
        const creditDuration = Date.now() - creditStartTime;
        rpcSuccess = true;
        
        console.log(`[WEBHOOK-${handlerId}] ✅✅✅ Step 6: Flex credits credited! (${creditDuration}ms)`, { 
          userId: user.id, 
          amount: creditAmount,
          snapshot: {
            credits_balance: creditResult.balance,
        ...
        console.log(`[WEBHOOK-${handlerId}] ✅ Verification - User credits after credit:`, verifyUser);
      } catch (creditErr) {
        rpcErrorDetails = {
          exception: true,
          message: creditErr instanceof Error ? creditErr.message : String(creditErr),
          stack: creditErr instanceof Error ? creditErr.stack : undefined,
        };
        console.error(`[WEBHOOK-${handlerId}] ❌❌❌ CRITICAL: Failed to credit flex credits:`, {
          error: creditErr instanceof Error ? creditErr.message : 'Unknown error',
          userId: user.id,
          amount: creditAmount,
        });
