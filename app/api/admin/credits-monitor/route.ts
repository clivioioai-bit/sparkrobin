  ...
  try {
    ...
    switch (action) {
      case 'add':
        ...
        try {
          const addResult = await creditCredits(
            userData.id,
            credits,
            'admin_add_credits',
            {
          ...
          return NextResponse.json({
            ...
            data: {
              credits_balance: addResult.balance,
              credits_total: addResult.total,
              credits_spent: addResult.spent
            }
          });
        } catch (addError) {
          console.error('Error adding user credits:', addError);
          return NextResponse.json(
            { error: 'Failed to add credits', details: addError instanceof Error ? addError.message : 'Unknown error' },
            { status: 500 }
          );
        }
