...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(`[API] Credits deducted - userId: ${userId}, amount: ${creditCost}, new balance: ${debitResult.balance}`);

    } catch (creditError) {
      console.error(`[API ERROR] Credit deduction failed - userId: ${userId}:`, creditError);
      
      if (creditError instanceof Error && creditError.message === 'INSUFFICIENT_BALANCE') {
        return NextResponse.json(
    ...
    // Call Kie.ai Gemini Omni Flash Pro Storyboard API
    try {
      ...
      // Use Gemini Omni Flash Pro Storyboard endpoint
      const kieApiUrl = `${apiBase}/api/v1/jobs/createTask`;
      
      console.log(`[API] Calling Kie.ai Gemini Omni Flash Pro Storyboard API: ${kieApiUrl}`);
      ...
      // Handle Kie.ai API response format according to Gemini Omni Flash Pro Storyboard spec
      ...
      return NextResponse.json({
        ...
        status: 'pending',
        message: 'Gemini Omni Flash Pro Storyboard generation started'
      });

    } catch (apiError) {
      console.error(`[API ERROR] Kie.ai Gemini Omni Flash Pro Storyboard API call failed - userId: ${userId}:`, apiError);
      ...
      let userMessage = 'Gemini Omni Flash Pro Storyboard generation failed';
