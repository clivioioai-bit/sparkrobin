...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call Kie.ai Spark Robin Pro Storyboard API
    try {
      ...
      // Use Spark Robin Pro Storyboard endpoint
      const kieApiUrl = `${apiBase}/api/v1/jobs/createTask`;
      
      console.log(`[API] Calling Kie.ai Spark Robin Pro Storyboard API: ${kieApiUrl}`);
      ...
      return NextResponse.json({
        ...
        status: 'pending',
        message: 'Spark Robin Pro Storyboard generation started'
      });

    } catch (apiError) {
      console.error(`[API ERROR] Kie.ai Spark Robin Pro Storyboard API call failed - userId: ${userId}:`, apiError);
      ...
      let userMessage = 'Spark Robin Pro Storyboard generation failed';
