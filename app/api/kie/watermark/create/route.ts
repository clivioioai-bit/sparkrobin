...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredCredits = 5; // fixed cost per watermark removal
    ...
    try {
      const debitResult = await debitCredits(
        ...
        requiredCredits,
        'watermark_remove',
        { video_url, status: 'processing' }
      );

      const remaining = debitResult.balance;
      ...
      return NextResponse.json({
        ...
        user_credits_remaining: remaining,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      });
    } catch (e) {
